...
class WalletConnect {
    ...
    func connect(session: WCSession, chainId: Int, walletId: String, uuid: UUID = UUID(), completion: @escaping ((Bool) -> Void)) {
        let clientMeta = WCPeerMeta(name: Strings.tokenary, url: "https://www.balance.io", description: Strings.walletConnectClientDescription, icons: ["https://www.balance.io/icon.png"])
