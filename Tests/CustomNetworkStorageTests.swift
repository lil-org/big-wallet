...
final class CustomNetworkStorageTests: XCTestCase {
    ...
    func testPartiallyCorruptArchiveSalvagesValidRecordsAndRepairsOnInsertion() throws {
        ...
        let initialSnapshot = try loadedSnapshot(from: defaults)
    ...
    func testInsertionUsesOneArchiveAndSnapshotUsesArchivedRPC() throws {
        ...
        let snapshot = try loadedSnapshot(from: defaults)
    ...
    func testInsertionStoresValidHTTPAndHTTPSURLsAndReloadsPreferredEndpoint() throws {
        ...
        XCTAssertEqual(
            try loadedSnapshot(from: defaults)
    ...
    func testLegacyOverrideRescuesDamagedLastDuplicateAndSurvivesRewrite() throws {
        ...
        let initial = try loadedSnapshot(from: defaults)
        ...
        let rewritten = try loadedSnapshot(from: defaults)
    ...
    private func loadedSnapshot(
        ...
        line: UInt = #line
    ) throws -> CustomNetworkSnapshot {
        let snapshot: CustomNetworkSnapshot?
        if case .loaded(let loaded) = SharedDefaults.loadCustomNetworkSnapshotResult(
            from: defaults
        ) {
            snapshot = loaded
        } else {
        ...
        return try XCTUnwrap(
            snapshot,
            "Expected a loaded custom-network snapshot",
            file: file,
            line: line
        )
    }
