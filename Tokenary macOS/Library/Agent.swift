...
class Agent: NSObject {
    ...
    private lazy var statusImage = Images.status
    ...
    func showInitialScreen(externalRequest: ExternalRequest?) {
        ...
        if case let .safari(request) = request {
            ...
            let onSelectedWalletClosure: ((EthereumChain?, TokenaryWallet?) -> Void)?
            if case let .wcSession(session) = request {
                onSelectedWalletClosure = onSelectedWallet(session: session)
            } else {
                onSelectedWalletClosure = nil
            }
            let windowController = Window.showNew()
            let accountsListVC = AccountsListAssembly.build(
                for: .mainScreen, onSelectedWallet: onSelectedWalletClosure
            )
