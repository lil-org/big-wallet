...
const MAX_RETAINED_REQUESTS = WORKFLOW_POLICY.maximumRetainedRequests;
...
var isCorrelatedDappResponse = BigWalletBridgeWire.isCorrelatedDappResponse;
var isRequestToken = BigWalletBridgeWire.isRequestToken;
var isNativeEnqueueAcknowledgement = BigWalletBridgeWire.isNativeEnqueueAcknowledgement;
...
function nativeMessage(subject, id, payload, requestToken, reviewToken) {
...
function scheduleNativeMessage(kind, subject, id, payload, requestToken, options = {}) {
    ...
    const operation = predecessor.then(async () => {
        ...
        try {
            const response = subject === "approveRequest"
                ? await pendingResponse
                : await settleNativeMessage(pendingResponse);
...
async function settleNativeMessage(pendingResponse) {
    return withTimeout(pendingResponse, NATIVE_MESSAGE_TIMEOUT);
}
...
function renderIdleSwitchControls(pending) {
    ...
    setHidden("idle-check-status", true);
...
function hasValidOptionalApprovalFields(state) {
    return isOptionalString(state.title) &&
        ...
        isOptionalBoolean(state.canUsePassword) &&
...
function isRenderableApprovalState(state, request) {
    ...
    if (
        ...
        typeof state.host !== "string" || state.host.length === 0 ||
        typeof state.canUsePassword !== "boolean") {
        return false;
    }
    switch (state.kind) {
        case "selectAccount":
...
function isCompactApprovalErrorState(state) {
    return isRecord(state) && state.state === "error" &&
        ...
        (typeof state.host === "undefined" ||
            typeof state.host === "string" && state.host.length > 0) &&
        Object.keys(state).every(key =>
            key === "id" || key === "state" || key === "host" || key === "error"
        );
}
...
function renderState(state) {
    ...
    setHidden("password-row", !state.canUsePassword);
...
async function approveCurrent() {
    ...
    if (approvalLifecycle.current.canUsePassword && passwordInput.value) {
...
async function revisionsForApproval(request) {
    let pending;
    try {
        pending = browser.runtime.sendMessage({
    ...
    if (outcome.status !== "response" ||
        !hasExactKeys(outcome.response, ["revisions"]) ||
        !isProviderRevisions(outcome.response.revisions)) {
        return null;
    }
    return {...outcome.response.revisions};
}
...
async function submitCurrentDecision(subject, payload) {
    ...
    if (subject === "approveRequest") {
        ...
        if (state.kind === "addChain") {
            delete decisionPayload.revisions;
        } else {
            const revisions = await revisionsForApproval(request);
            if (!remainsCurrent()) {
                rerenderCurrentReview();
                return;
            }
            ...
            decisionPayload.revisions = revisions;
        }
    }
    approvalLifecycle.generation += 1;
    stopTimers();
    const ticket = scheduleNativeMessage(
        ...
        decisionPayload,
        request.requestToken,
        {isValid: remainsCurrent, reviewToken}
    );
    const outcome = await ticket.result;
...
async function applySuggested() {
    if (approvalLifecycle.current?.transactionMutationAllowed !== true) { return; }
...
async function switchAccountFromIdle() {
    ...
    if (manualSwitchAttempt && !sameTab(manualSwitchAttempt.tab, tab)) {
        manualSwitchAttempt = null;
    }
    if (!manualSwitchAttempt) {
        ...
        const id = genId();
        const enqueueAttempt = genPrivateToken();
        manualSwitchAttempt = { tab, id, message: {
            name: "switchAccount",
            id: id,
            enqueueAttempt: enqueueAttempt,
            admissionDeadline: Date.now() +
                WORKFLOW_POLICY.requestTTLMilliseconds,
            expectedConfigurationKey: tab.configurationKey,
            message: {
                id: id,
                name: "switchAccount",
                provider: "unknown",
                body: { latestConfigurations: configuration.latestConfigurations },
            },
        } };
    }
    const { id, message } = manualSwitchAttempt;
    let pending;
    try {
    ...
    if (!isCorrelatedDappResponse(response, id) &&
        !isNativeEnqueueAcknowledgement(response, id)) {
        setText("idle-connection", localized("failedToLoad", "Failed to load"));
        button.disabled = false;
        renderIdleSwitchControls(false);
        return;
