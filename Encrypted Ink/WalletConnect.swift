...
class WalletConnect {
    ...
    func connect(session: WCSession, chainId: Int, walletId: String, uuid: UUID = UUID(), completion: @escaping ((Bool) -> Void)) {
        let clientMeta = WCPeerMeta(name: Strings.encryptedInk, url: "https://encrypted.ink", description: Strings.walletConnectClientDescription, icons: ["https://encrypted.ink/icon.png"])
